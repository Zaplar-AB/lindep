...
fn event_loop(terminal: &mut DefaultTerminal, app: &mut App, mut rx: AppEventRx) -> io::Result<()> {
    ...
    while !app.should_quit {
        ...
        // `term_event::poll` only wakes early on terminal input, so a background
        // AppEvent (needs-you, a spawn, a hook notification) isn't visible until
        // the next poll returns. While attached we poll fast so keystrokes and
        // PTY output feel live; idle, we hold a short 50 ms cadence so a "needs
        // you" prompt or notification lights up the node promptly — still far
        // longer than a frame, so a quiet cockpit never busy-repaints (only the
        // dirty flag triggers a redraw, and an idle tick sets nothing dirty).
